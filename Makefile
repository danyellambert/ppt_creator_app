...
.PHONY: install install-dev test lint format validate-example render-example render-all-examples review-example review-pptx-example playground generate-briefing-example docker-render-example gallery layout-audit ai-benchmark build-dist check-dist release-smoke ci
