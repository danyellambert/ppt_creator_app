...
def render(renderer, slide, slide_spec, meta, index, total_slides) -> None:
    ...
    panel_bounds = renderer.build_grid_bounds(
        ...
        height=height,
        column_regions=[{"kind": "column", "min_width": 3.6, "flex": 1.0} for _ in slide_spec.two_column_columns],
    ...
    for idx, (column, (left, panel_top, panel_width, panel_height)) in enumerate(zip(slide_spec.two_column_columns, panel_bounds, strict=True)):
        ...
        regions.append({"kind": "title", "height": 0.40})
        if column.body:
            regions.append({"kind": "body", "height": 0.88 if column.bullets else 1.55})
        if column.bullets:
